...
class Exam(TimestampMixin, Deletable, Stageable, Sortable, models.Model):
    ...
    def student_time_remaining(self, student):
        """
        Returns timedelta object representing student's time remaining on an exam, where the
        endpoint is either time limit on a timed exam or the exam's due date, which ever is earlier.
        """
        #get the endpoint
        if not self.timed:
            endpoint = self.due_date
        elif not StudentExamStart.filter(student=student, exam=self).exists(): #student has not started
            timed_endpoint = datetime.now() + datetime.timedelta(minutes=self.minutesallowed)
            endpoint = timed_endpoint if timed_endpoint < self.due_date else self.due_date
        else: #student has started
            timed_endpoint = StudentExamStart.get(student=student, exam=self).time_created + \
                             datetime.timedelta(minutes=self.minutesallowed)
            endpoint = timed_endpoint if timed_endpoint < self.due_date else self.due_date
        return endpoint - datetime.now()
