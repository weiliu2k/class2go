...
def checkgood(userquery, dbname, ansfile, qnum):
    ...
    try:
        if exists(ansfile):
            (ans, order, checkquery) = getanswermod(ansfile, qnum, dbname)
        else:
...
if __name__ == '__main__':
    ...
    try:
        (correct, qr, ans, order, checkquery) = checkgood(userquery, dbname, ansfile, qnum)
    except:
        from sys import exc_info
        e = exc_info()
        scorestr = 'Score: 0'
