...
if __name__ == '__main__':
    ...
    try:
        (correct, qr, ans, order, checkquery, checkmod, trigsuccess, modsuccess, moddesc) = checkgood(usertrig, dbname, ansfile, qnum)
    except:
        from sys import exc_info
        e = exc_info()
        scorestr = 'Score: 0'
    ...
    try:
        ...
        if usertrig != '':
            if trigsuccess:
                ...
                feedbackstr += '<br/>Trigger command(s) were executed.<br/>'

                if modsuccess:
                #if usertrig != '' and not isinstance(qr, str):
                    feedbackstr += '<br/>To check your trigger(s), we first ran the following data modification statement(s): <i>%s</i>. ' % checkmod
                    if moddesc != '':
                        feedbackstr += '<br/>English description of modifications: ' + moddesc + ' '
                    feedbackstr += '<br/><br/>We then ran the following query: <i>%s</i><br/>' % (checkquery)
                    feedbackstr += '<br>Your Query Result: ' + toTable(qr) + ' '

                    if usertrig != '' and not isinstance(qr, str):
                        feedbackstr += '<br>Expected Query Result: ' + toTable(ans) + ' '
                else:
                    feedbackstr += '<br/>To check your trigger(s), we tried to run the following data modification statement(s): <i>%s</i>. <br/>' % (checkmod)
                    if moddesc != '':
                        feedbackstr += '<br/>English description of modifications: ' + moddesc + ' '
